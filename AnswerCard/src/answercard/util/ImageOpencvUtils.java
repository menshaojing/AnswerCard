...
public class ImageOpencvUtils {
	...
	//A4像素尺寸 96 分辨率 794,1123
	// 300分辨率 2480 3508
	// 200分辨率 1652 2338
	// 150分辨率 1240 1754
	// 100分辨率 827  1098
	// 120分辨率 992  1403
	private static final Size A4_SIZE=new Size(1240,1754);
	//420mm*297mm
	private static final int A3_WIDTH=420;
	private static final int A3_HEIGHT=297;
	//A3像素尺寸
	private static final Size A3_SIZE=new Size(2480,1754);
    //定位点矩形实际尺寸 6mm*4mm
	private static final int LOCATION_POINT_WIDTH=6;
	private static final int LOCATION_POINT_HEIGHT=4;
	//定位点矩形上边距 上下是8mm
	private static final int LOCATION_POINT_TOP_MARGIN=8;
	//定位点矩形上边距 左右 9mm
	private static final int LOCATION_POINT_LEFT_MARGIN=9;
     //300 分辨率 11.8
	 // 200 分辨率 7.88
	// 150 分辨率 5.90
	// 100 分辨率 3.97
	// 120 分辨率 4.72
	static double pixel = 5.90;
	// 私有化构造函数
     ...
     * @Date  2021/5/20  8:55
     **/
	public static Mat restoreImage(Mat original,Mat current){
		//设置A3或者A4大小
		Mat mat =   installSize(original);
		//设置实际内容中心尺寸  W : 原图像尺寸宽 - 定点矩形宽 - 定点左边距*2   H：原图像尺寸宽 - 定点矩形高 - 定点上边距*2
		double width =mat.width() - LOCATION_POINT_LEFT_MARGIN * pixel * 2;
		double height = mat.height()-LOCATION_POINT_TOP_MARGIN * pixel * 2;
		Mat currentMat = new Mat(new Size(width, height), current.type());
		Imgproc.resize(current, currentMat, currentMat.size(), 0, 0, Imgproc.INTER_CUBIC);
		//Imgcodecs.imwrite("D:/work/AnswerCard/AnswerCard/img/A3003.png", currentMat);
        //进行裁剪的图像填充原尺寸 1.在原图像尺寸中找出裁剪位置 2. 进行填充
	 ...
	 * @Date  2021/5/21  9:48
	 **/
	static Mat installSize(Mat original){
	 	Mat mat = new Mat();
		if( original.width() >=  (int)A4_SIZE.width-500 && original.width() < (int)A4_SIZE.width +500){
			mat=new Mat(A4_SIZE,original.type(),SCALAR);
			//为了提高精度，重新设置像素比
			//pixel=A4_SIZE.width/A4_WIDTH;
		}
		if(original.width() >= (int) A3_SIZE.width-100 && original.width() <  (int)A3_SIZE.width +100){
			mat=new Mat(A3_SIZE,original.type(),SCALAR);
			//为了提高精度，重新设置像素比
			//pixel=A3_SIZE.width/A3_WIDTH;
		}
		return mat;
	}
	/**
