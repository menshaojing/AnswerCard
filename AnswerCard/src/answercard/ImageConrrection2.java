...
/**
 * @ClassName ImageConrrection2
 * @Description TODO
 * @Author menshaojing
 * @Date 2021/5/10 13:44
 * @Version 1.0
 */
public class ImageConrrection2 {

    public static void main(String[] args) {
        System.load("D:/work/AnswerCard/AnswerCard/libs/x64/opencv_java450.dll");
        String path="D:/work/AnswerCard/AnswerCard/img/TS.jpg";
        Mat srcImage = Imgcodecs.imread(path);

        Mat gray_image = srcImage.clone();

        // 彩色转灰度
        Imgproc.cvtColor(srcImage,gray_image,Imgproc.COLOR_BGR2GRAY);
        Imgcodecs.imwrite("D:/A3-ok12.jpg", gray_image);
        // 高斯滤波，降噪
        Imgproc.GaussianBlur(gray_image, gray_image, new Size(3,3), 2, 2);
        Imgcodecs.imwrite("D:/A3-ok13.jpg", gray_image);
        // Canny边缘检测
        Imgproc.Canny(gray_image, gray_image, 20, 60, 3, false);
        Imgcodecs.imwrite("D:/A3-ok14.jpg", gray_image);
        // 膨胀，连接边缘
        Imgproc.dilate(gray_image, gray_image, new Mat(), new Point(-1,-1), 3, 1, new Scalar(1));
        Imgcodecs.imwrite("D:/A3-ok15.jpg", gray_image);
        //轮廓提取
        List<MatOfPoint> contours = new ArrayList<>();
        Mat hierarchy = new Mat();
        Imgproc.findContours(gray_image, contours, hierarchy, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE);
        Imgcodecs.imwrite("D:/A3-ok16.jpg", gray_image);

       // 找出轮廓对应凸包的四边形拟合
        List<MatOfPoint> squares = new ArrayList<>(10);
        List<MatOfPoint> hulls = new ArrayList<>(10);
        ...
        Imgcodecs.imwrite("D:/maxCosine.jpg", gray_image);
        // 找出外接矩形最大的四边形
...
// 如果提取出的图片宽小于高，则旋转90度
        ...
        Imgcodecs.imwrite("D:/A3-ok17.jpg", quad);
...
// 提取图像
        ...
        Imgproc.warpPerspective(largest_square, quad, transmtx, quad.size());
        Imgcodecs.imwrite("D:/A3-ok18.jpg", quad);
    }
